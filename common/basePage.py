...
class BasePage():
    ...
    def refresh(self):
        '''刷新浏览器'''
        self.driver.refresh()
        self.logger.info("网址已经刷新")

    def back(self):
        '''返回'''
        self.driver.back()
        self.logger.info("返回上页面")
    ...
    # def js_showtime(self):
    #     '''设置修改标签内属性'''
    #     self.logger.info("调用JS执行修改标签内属性")
    #     try:
    #         js = "document.querySelector('body > div:nth-child(12) > div > div.ant-modal-wrap.ant-modal-wrap-center.adjustment > div > div.ant-modal-content > div.ant-modal-body > div > form > ul > li.adjust-plan__list > ul > li > div.ant-select.ant-select-enabled > div').setAttribute('aria-expanded','true')"
    #         self.driver.execute_script(js)
    #         self.logger.info("修改成功")
    #     except Exception:
    #         self.get_screen()
